...
import { CHECKLIST_GROUPS, ChecklistGroup, ChecklistKey } from '../workflow/kistlchecklist.config'
...
import { prepareCardForEditing, getErrorMessage } from '../workflow/card-utils'

import { isPlanningCard } from '../workflow/actions'

import { getPreviousStepPatch } from '../workflow/kistl-previous-step'

import { approveReviewPatch, declineReviewPatch } from '../workflow/review-actions'
...
export class CardModalStore {
  ...
  open(card: KistlCard): void {
    ...
    this.selectedCard.set(prepareCardForEditing(card))
  }
    ...
    this.kistlboard.saveDetails(card).subscribe({
      next: (updatedCard) => {
        this.selectedCard.set(
          prepareCardForEditing(mergeUpdatedCardWithCurrentAssets(card, updatedCard)),
        )
      ...
      error: (error) => {
        ...
        this.error.set(getErrorMessage(error, 'Karte konnte nicht gespeichert werden.'))
  ...
  toggleChecklistItem(card: KistlCard, key: ChecklistKey, event: Event): void {
    ...
    this.kistlboard.toggleChecklistItem(card, key, input.checked).subscribe({
      ...
      error: (error) => {
        ...
        this.error.set(getErrorMessage(error, 'Checklist konnte nicht aktualisiert werden.'))
    ...
    this.kistlboard
      ...
      .subscribe({
        ...
        error: (error) => {
          ...
          this.error.set(getErrorMessage(error, 'Workflow konnte nicht gestartet werden.'))
  ...
  moveSelectedCardToPreviousStep(): void {
    ...
    this.kistlboard.updateCard(card.id, result.patch).subscribe({
      ...
      error: (error) => {
        ...
        this.error.set(getErrorMessage(error, 'Karte konnte nicht zurückgesetzt werden.'))
  ...
  archiveSelectedCard(): void {
    ...
    this.kistlboard.archive(card).subscribe({
      ...
      error: (error) => {
        ...
        this.error.set(getErrorMessage(error, 'Karte konnte nicht archiviert werden.'))
  ...
  uploadMediaFiles(card: KistlCard, event: Event): void {
    ...
    merge(
      ...
      complete: () => {
        ...
        if (currentCard) {
          this.selectedCard.set(prepareCardForEditing(addAssetsToCard(currentCard, uploadedMedia)))
        }
  ...
  getFinalVideoOptions(card: KistlCard): KistlMedia[] {
    return this.getAssets(card).filter((asset) => asset.assetType === 'final-video')
  }

  getSelectedFinalVideoId(card: KistlCard): string | number | null {
    if (!card.finalVideo) {
      return null
    }

    if (typeof card.finalVideo === 'string' || typeof card.finalVideo === 'number') {
  ...
  setFinalVideo(card: KistlCard, mediaId: string | number): void {
    this.error.set('')
    this.saving.set(true)
    ...
    this.kistlboard
      ...
      .subscribe({
        ...
        error: (error) => {
          ...
          this.error.set(getErrorMessage(error, 'Final Video konnte nicht gesetzt werden.'))
  ...
  // Review actions
  // -------------------------
  approveReview(card: KistlCard): void {
    this.error.set('')
    this.saving.set(true)

    this.kistlboard.updateCard(card.id, approveReviewPatch(card)).subscribe({
      next: (updatedCard) => {
        this.selectedCard.set(
          prepareCardForEditing(mergeUpdatedCardWithCurrentAssets(card, updatedCard)),
        )
      ...
      error: (error) => {
        ...
        this.error.set(getErrorMessage(error, 'Review konnte nicht approved werden.'))
  ...
  declineReview(card: KistlCard): void {
    if (!card.review?.comment?.trim()) {
      this.error.set('Bitte Review-Kommentar eingeben.')
    ...
    this.kistlboard.updateCard(card.id, declineReviewPatch(card)).subscribe({
      next: (updatedCard) => {
        this.selectedCard.set(
          prepareCardForEditing(mergeUpdatedCardWithCurrentAssets(card, updatedCard)),
        )
      ...
      error: (error) => {
        ...
        this.error.set(getErrorMessage(error, 'Review konnte nicht declined werden.'))
